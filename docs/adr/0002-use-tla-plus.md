...
The correctness risks in a bond exchange are dominated by behavior over time:
orders race with cancellation, matching changes ownership, settlement crosses
service boundaries, retries may duplicate work, and partial failures expose
intermediate states. These risks arise from possible action interleavings and
cannot be addressed completely by validating data structures or testing a few
...
small, finite model configurations and check safety and liveness properties.
...
- Nondeterministic action interleavings and failure scenarios can be explored
  before production code exists.
- Safety properties such as conservation, uniqueness, and idempotency, and
  liveness properties such as eventual settlement, can share one model.
...
It was not selected as the primary tool because the exchange's initial risks
are action ordering, concurrency, retries, and progress. TLA+'s initial-state
and next-state style maps more directly to those questions. Alloy remains a
reasonable supplementary tool if complex relational structure becomes the
dominant modeling concern.
